...
export class Asteroid extends Entity implements CircleCollider {
  ...
  constructor(position: Vec2, size: AsteroidSize = "L") {
    super(position);
    this.size = size;
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.moveTo(Math.cos(first.angle) * r * first.scale, Math.sin(first.angle) * r * first.scale);
  ...
  split(): Asteroid[] {
    ...
    const a = new Asteroid({ x: this.position.x + dx, y: this.position.y + dy }, next);
    const b = new Asteroid({ x: this.position.x - dx, y: this.position.y - dy }, next);

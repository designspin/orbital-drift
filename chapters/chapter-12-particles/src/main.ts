import './style.css'
import { AssetManager, AudioManager, Camera, Engine, ParticleSystem, ShieldBar, StateMachine, UILayer, UIPanel, ScoreDisplay, TextDisplay } from '@course/lib';
...
class CH12 extends Engine {
  ...
  public updatePlaying(deltaTime: number): void {
    ...
    if (this.input.wasPressed(' ')) {
      ...
      this.emitMuzzleFlash(spawn, angle);
    }
    ...
    if (!this.player.alive) {
      if (!this.playerDeathExploded) {
        ...
        this.emitExplosion(this.player.position, 30, '#7fd1ff', '#ffffff');
      }
      if (this.lives > 0) {
        this.lives -= 1;
        if (this.lives > 0) {
  ...
  private onEnemyDestroyed(position: Vec2): void {
    this.enemyKills += 1;

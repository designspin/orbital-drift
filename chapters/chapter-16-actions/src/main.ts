...
class CH16 extends Engine {
  ...
  private setupActions(): void {
    ...
    this.input.bindAction('shield', 'ArrowDown', 's', 'S');
    this.input.bindAction('fire', ' ', 'Space');
  ...
  private setupStates(): void {
    ...
    const gameOverState: GameState = {
      ...
      update: (dt) => {
        ...
        if (this.input.wasActionPressed('confirm') || this.input.wasActionPressed('fire') || this.gameOverTime >= 3) {
          this.stateMachine.set('menu');
        }
      },
      render: () => {
